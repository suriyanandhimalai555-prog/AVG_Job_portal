        ...
        <div className="max-w-7xl mx-auto space-y-4 p-4 rounded-2xl shadow-sm bg-[#EEF2FF] relative">
            ...
            <div className="flex bg-white border border-[#EBEBEB] p-1 rounded-xl shadow-sm">
                <button
                    ...
                    className={`flex-1 font-bold py-2.5 rounded-lg text-sm transition-all ${activeTab === 'browse' ? 'bg-blue-50 text-[#2A45C2] shadow-sm border border-[#2A45C2]/10' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    Browse Courses
                </button>
                <button
                    onClick={() => setActiveTab('learning')}
                    className={`flex-1 font-bold py-2.5 rounded-lg text-sm transition-all ${activeTab === 'learning' ? 'bg-blue-50 text-[#2A45C2] shadow-sm border border-[#2A45C2]/10' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    My Learning ({enrolledCourses.size})
                </button>
                <button
                    onClick={() => setActiveTab('wishlist')}
                    className={`flex-1 font-bold py-2.5 rounded-lg text-sm transition-all flex items-center justify-center gap-2 ${activeTab === 'wishlist' ? 'bg-blue-50 text-[#2A45C2] shadow-sm border border-[#2A45C2]/10' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    <FaHeart className={activeTab === 'wishlist' ? 'text-[#2A45C2]' : 'text-red-400'} /> Wishlist ({favorites.size})
                </button>
            </div>

            <div className="bg-white p-4 rounded-xl shadow-sm border border-[#EBEBEB] space-y-3">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-11 pr-4 py-2.5 bg-gray-50 border border-transparent rounded-lg focus:outline-none focus:border-[#2A45C2] focus:bg-white transition-colors text-sm font-medium"
                    />
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <select
                        ...
                        className="flex-1 p-2.5 bg-gray-50 border border-transparent rounded-lg text-sm font-bold text-gray-700 focus:outline-none focus:border-[#2A45C2] focus:bg-white transition-colors"
                    >
            ...
            <div>
                <div className="flex justify-between items-center mb-4 px-1 mt-2">
                    <h2 className="text-lg font-extrabold text-gray-900">{getTabTitle()}</h2>
                ...
                {isLoading ? (
                    ...
                    <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5" : "flex flex-col gap-4"}>
                        {filteredCourses.map((course) => (
                            <div
                                ...
                                className={`bg-white border border-[#EBEBEB] rounded-xl overflow-hidden flex cursor-pointer hover:shadow-lg transition-all group relative ${viewMode === 'grid' ? 'flex-col' : 'flex-col sm:flex-row h-auto sm:h-48'}`}
                            >
                                ...
                                {course.is_featured && (
                                    <div className="absolute top-3 left-3 z-10 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white text-[10px] font-black px-2.5 py-1 rounded shadow-sm flex items-center gap-1 uppercase tracking-wider">
                                        <FaStar size={10} /> Featured
                                    </div>
                                )}

                                <div className={`${viewMode === 'grid' ? 'w-full h-44' : 'w-full sm:w-64 h-48 sm:h-full'} bg-gray-100 relative overflow-hidden flex-shrink-0`}>
                                    {course.thumbnail_url ? (
                                        ...
                                        <div className="w-full h-full bg-[#2A45C2]/10 flex flex-col items-center justify-center text-[#2A45C2]">
                                            <FaPlayCircle size={40} className="opacity-50 mb-2" />
                                ...
                                <div className="flex-1 flex flex-col justify-between p-4 sm:p-5">
                                    <div>
                                        ...
                                        <h3 className="text-md sm:text-lg font-black text-gray-900 leading-tight mb-2 group-hover:text-[#2A45C2] transition-colors line-clamp-2">{course.title}</h3>

                                        <div className="flex items-center text-xs font-bold gap-1.5 mb-3 text-gray-600">
                                    ...
                                    <div className="mt-auto pt-3 border-t border-[#EBEBEB] flex items-center justify-between">
                                        <div className="flex flex-col">
                                            {course.discount_price ? (
                                                <>
                                                    <span className="font-black text-gray-900 text-lg">{course.discount_price}</span>
                                        ...
                                        <Button className="rounded-lg px-6 py-2 bg-[#2A45C2] text-white border-0 font-bold text-sm shadow-sm hover:scale-105 transition-transform">
                    ...
                    <div className="text-center py-12 bg-white border border-[#EBEBEB] rounded-xl shadow-sm">
                        {activeTab === 'wishlist' && favorites.size === 0 ? (
                            <p className="text-gray-500 font-medium mb-3">Your database wishlist is currently empty.</p>
                        ) : activeTab === 'learning' && enrolledCourses.size === 0 ? (
                            <p className="text-gray-500 font-medium mb-3">You haven't enrolled in any courses in the database yet.</p>
                        ) : (
                            <>
                                <p className="text-gray-500 font-medium mb-3">No courses found matching your criteria.</p>
            ...
            {isDetailsModalOpen && selectedCourse && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-gray-900/70 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[95vh] sm:max-h-[90vh] flex flex-col relative overflow-hidden">
                        ...
                        <div className="w-full h-64 sm:h-80 bg-black relative flex-shrink-0">
                            {selectedCourse.preview_video_url ? (
                                ...
                                <div className="w-full h-full relative">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="flex flex-col items-center text-white">
                                            <FaPlayCircle size={60} className="mb-2 opacity-80 cursor-not-allowed" />
                                ...
                                <div className="w-full h-full flex flex-col items-center justify-center text-gray-500">
                                    <FaPlayCircle size={60} className="mb-2 opacity-50" />
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col md:flex-row bg-gray-50">
                            ...
                            <div className="w-full md:w-2/3 p-6 sm:p-8 space-y-8 bg-white">
                                <div>
                                    ...
                                    <h1 className="text-2xl sm:text-4xl font-black text-gray-900 leading-tight mb-4">{selectedCourse.title}</h1>
                                ...
                                <section className="border border-[#EBEBEB] rounded-xl p-5 bg-gray-50 flex items-start gap-4">
                                    <div className="w-16 h-16 bg-white border border-[#EBEBEB] rounded-full flex items-center justify-center text-gray-300 flex-shrink-0 shadow-sm">
                                        <FaUserCircle size={40} />
                                    </div>
                                    <div>
                                        <h3 className="text-sm font-extrabold text-gray-900 uppercase tracking-wider mb-1">Meet Your Instructor</h3>
                                        <p className="font-bold text-[#2A45C2] text-lg">{selectedCourse.instructor_name || 'Expert Faculty'}</p>
                                ...
                                <section>
                                    <h3 className="text-sm font-extrabold text-gray-900 uppercase tracking-wider mb-4 border-b border-[#EBEBEB] pb-2">What's Included</h3>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="flex items-center gap-3 text-sm text-gray-700">
                                            <div className="p-2 bg-blue-50 text-[#2A45C2] rounded-lg"><FaClock size={16} /></div>
                                            <span className="font-bold">{selectedCourse.duration || 'Self-paced'}</span>
                                        </div>
                                        <div className="flex items-center gap-3 text-sm text-gray-700">
                                            <div className="p-2 bg-blue-50 text-[#2A45C2] rounded-lg"><FaBookOpen size={16} /></div>
                                            <span className="font-bold">{selectedCourse.lessons_count} Comprehensive Lessons</span>
                                        </div>
                                        <div className="flex items-center gap-3 text-sm text-gray-700">
                                            <div className="p-2 bg-blue-50 text-[#2A45C2] rounded-lg"><FaGlobe size={16} /></div>
                                            <span className="font-bold">Language: {selectedCourse.language}</span>
                                        </div>
                                        {selectedCourse.has_certificate && (
                                            <div className="flex items-center gap-3 text-sm text-gray-700">
                                                <div className="p-2 bg-green-50 text-green-600 rounded-lg"><FaCertificate size={16} /></div>
                                                <span className="font-bold text-green-700">Verifiable Certificate of Completion</span>
                                            </div>
                                        )}
                                        {selectedCourse.start_date && (
                                            <div className="flex items-center gap-3 text-sm text-gray-700">
                                                <div className="p-2 bg-blue-50 text-[#2A45C2] rounded-lg"><FaCalendarAlt size={16} /></div>
                                ...
                                <section>
                                    <div className="flex items-center justify-between border-b border-[#EBEBEB] pb-2 mb-4">
                                        <h3 className="text-sm font-extrabold text-gray-900 uppercase tracking-wider">Student Reviews</h3>
                                        <div className="flex items-center gap-2">
                                            <FaStar className="text-yellow-400" size={16} />
                                            <span className="font-black text-lg text-gray-900">{getMockRating(selectedCourse.id)}</span>
                                    ...
                                    <div className="space-y-4">
                                        {[
                                            ...
                                            <div key={idx} className="border-b border-[#EBEBEB] last:border-0 pb-4 last:pb-0">
                            ...
                            <div className="w-full md:w-1/3 bg-gray-50 border-l border-[#EBEBEB] p-6 sm:p-8 flex flex-col gap-6 sticky top-0">

                                <div className="bg-white p-6 rounded-2xl border border-[#EBEBEB] shadow-lg text-center">
                                    ...
                                    {selectedCourse.discount_price ? (
                                        <div className="flex flex-col items-center mb-4">
                                            <span className="text-4xl font-black text-gray-900">{selectedCourse.discount_price}</span>
                                        ...
                                        <div className="text-4xl font-black text-gray-900 mb-6">{selectedCourse.price}</div>
                                    )}

                                    <div className="space-y-3">
                                        <Button
                                            onClick={() => handleEnroll(selectedCourse.id, selectedCourse.title)}
                                            className={`w-full py-3.5 text-white border-0 font-black text-md shadow-md transition-transform ${enrolledCourses.has(selectedCourse.id) ? 'bg-green-600' : 'bg-[#2A45C2] hover:-translate-y-0.5'}`}
                                        >
                                            {enrolledCourses.has(selectedCourse.id) ? 'Continue Learning' : 'Enroll Now'}
                                        </Button>
                                        <Button onClick={(e) => toggleFavorite(e, selectedCourse.id)} variant="outline" className={`w-full py-3 font-bold border-2 ${favorites.has(selectedCourse.id) ? 'border-red-200 text-red-500 bg-red-50 hover:bg-red-100' : 'border-[#EBEBEB] text-gray-700 bg-white hover:bg-gray-50'}`}>
                                            {favorites.has(selectedCourse.id) ? 'Saved to Wishlist' : 'Add to Wishlist'}
                                        </Button>
                                    </div>
                                    <p className="text-[10px] text-gray-400 font-medium mt-4">30-Day Money-Back Guarantee</p>
                                </div>

                                <div className="bg-white p-5 rounded-xl border border-[#EBEBEB] shadow-sm">
                                    <h4 className="font-extrabold text-gray-900 text-sm mb-2 flex items-center gap-2"><FaFileDownload className="text-[#2A45C2]" /> Course Brochure</h4>
                                    <p className="text-xs text-gray-500 mb-4">Download the full syllabus, module breakdowns, and technical prerequisites.</p>
                                ...
                                {selectedCourse.has_certificate && (
                                    <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-5 rounded-xl shadow-md text-white">
                                        <FaCertificate className="text-yellow-400 text-2xl mb-2" />
